...
if ($_SESSION['user_role'] != "Super Admin") {
    ...
    if ($_SESSION['supplier_form_create'] == 0) {
?>
<script>
document.addEventListener("DOMContentLoaded", function() {
    let submitBtn = document.getElementById('submitSupBtn');
    if (submitBtn) {
...
<?php
    ...
    if ($_SESSION['supplier_form_reprint'] == 0) {
    ?>
<script>
document.addEventListener('DOMContentLoaded', function() {

    let supSearchBtn = document.getElementById("supplierSearchBtn");
    if (supSearchBtn) {
        supSearchBtn.setAttribute('disabled', true)
    }

    let supSearch = document.getElementById("supplierSearch");
    if (supSearch) {
...
<?php
    ...
    if ($_SESSION['supplier_form_update'] == 0) {
    ?>
...
<?php
    ...
    if ($_SESSION['supplier_form_delete'] == 0) {
    ?>
<script>
document.addEventListener("DOMContentLoaded", function() {
...
$_SESSION['searching_name'] = "";
$_SESSION['table_name'] = "suppliers";
$_SESSION['field_name'] = "supplier_name";
...
if (isset($_POST['createSupplier'])) {
    try {
        ...
        //    && $email !== '' && $gst !== '') {
        if (isset($name) && $name != '' && isset($state) && $state != '') {

            // Prepare the query to prevent SQL injection
            // Prepare the SQL statement

            $querySearchSupplier = "select*from suppliers where supplier_name = '$name'
                                    and branch_id = '$branchid'";
            $resultSearchSupplier = $con->query($querySearchSupplier);

            if ($resultSearchSupplier->num_rows == 0) {


                $queryCreateSupplier = "INSERT INTO suppliers (
                        supplier_name, 
                        address1, 
                        address2, 
                        address3, 
                ...
                $stmt = $con->prepare($queryCreateSupplier);
                $stmt->bind_param(
                    "ssssssssssssii",
                    $name,
                    $address1,
                ...
                if ($stmt->execute()) {
                    ...
                    header("Location:" . BASE_URL . "/pages/supplierMaster.php");
                    exit;
                } else {
                    $con->rollback();
                    $_SESSION['failure'] = "Failed to create supplier. Please try again.";
                }
            } else {
                $con->rollback();
                $_SESSION['failure'] = "Supplier Already Exist! Cannot Create A Duplicate";
            }
        } else {
            $_SESSION['failure'] = "Supplier Name And State Name Is Required To Create.";
        }
    } catch (mysqli_sql_exception $e) {
        // Check if it's a duplicate entry error
        $con->rollback();
        if ($e->getCode() == 1062) { // Error code for duplicate entry
            $_SESSION['failure'] = "Supplier Name '$name' already exists. Please enter a different supplier name.";
        } else {
...
if (isset($_GET['updationid'])) {
    try {
        ...
        // Extract GET data
        extract($_GET);
        $querySearchSupplier = "select*from suppliers where id = '$updationid'";
        $resultSearchSupplier = $con->query($querySearchSupplier)->fetch_assoc();
        $name     = $resultSearchSupplier['supplier_name'];
        $address1 = $resultSearchSupplier['address1'];
        $address2 = $resultSearchSupplier['address2'];
        $address3 = $resultSearchSupplier['address3'];
        $locality = $resultSearchSupplier['locality'];
        $city     = $resultSearchSupplier['city'];
        $pincode  = $resultSearchSupplier['pincode'];
        $state    = $resultSearchSupplier['state'];
        $landline = $resultSearchSupplier['landline'];
        $mobile   = $resultSearchSupplier['mobile'];
        $email    = $resultSearchSupplier['email'];
        $gst      = $resultSearchSupplier['gst_no'];
        ...
        if (isset($_POST['updateSupplier'])) {
            ...
            if (isset($name) && $name != '' && isset($state) && $state != '') {
                ...
                if ($resultSearchSupplier->num_rows == 0) {
                    ...
                    $qryUpdateSupplier = "UPDATE suppliers SET supplier_name = ?, address1 = ?,
                    address2 = ?, address3 = ?, locality = ?, city = ?,
                    pincode = ?, state = ?, landline = ?, mobile = ?,
                    email = ?, gst_no = ?, user_id = ?, branch_id = ? WHERE id = ?";
                    $stmt = $con->prepare($qryUpdateSupplier);
                    $stmt->bind_param(
                        "ssssssssssssiii",
                        $name,
                        $address1,
                    ...
                    if ($stmt->execute()) {
                        ...
                        $con->rollback();
                        $_SESSION['failure'] = "Failed to update supplier. Please try again.";
                    }
                } else {
                    $con->rollback();
                    $_SESSION['failure'] = "Supplier Already Exist! Cannot Create A Duplicate";
                ...
                $_SESSION['failure'] = "Supplier Name or State Name cannot be empty.";
            }
        }
    } catch (mysqli_sql_exception $e) {
...
//     $queryDeleteSupplier = "delete from suppliers where id='$deletionid'";
//     $resultDeleteSupplier = $con->query($queryDeleteSupplier);
...
if (isset($_GET['deletionid'])) {
    try {
        ...
        // Extract ID from GET request
        $deletionid = $_GET['deletionid'];

        // Prepare delete statement
        $queryDeleteSupplier = "DELETE FROM suppliers WHERE id = ?";
        $stmtDelete = $con->prepare($queryDeleteSupplier);
        ...
        if ($stmtDelete->execute()) {
            ...
            $con->rollback(); //  Rollback if something fails
            $_SESSION['failure'] = "Failed to delete supplier. Please try again.";
        }
...
if (isset($_POST['exportButton'])) {
    extract($_POST);
    if ($fileType == "pdfFile") {
        $_SESSION['report_title'] = "Suppliers Report";
        $_SESSION['table_header'] = "Supplier Name";
...
<form action="" class="supplierForm" id="supplierForm" method="post">
    <h3 style="text-align:left;font-family:Verdana, Geneva, Tahoma, sans-serif;"><span style="font-weight:bolder">Manage
        </span><span style="font-size:medium;font-weight:bold;color:gray;">Supplier</span></h3>
    <hr>
    ...
    <div style="display:flex;gap:10px;">
        ...
        <div class="form-floating">
            ...
            <label for="floatingInput">State <span style="color:red;font-size:20px;">*</span></label>
        </div>
    ...
    <div style="display:flex; gap:150px;margin-top:-10px;" id="beforeEditPress">
        <button type="submit" name="createSupplier" id="submitSupBtn" class="btn btn-primary"
    ...
    <div style="display:flex;gap:150px;margin-top:-38px;" id="afterEditPress">
        <button type="submit" name="updateSupplier" id="updateSupBtn" class="btn btn-primary"
            style="width:120px;">Update</button>
        <a href="<?php echo BASE_URL; ?>/pages/supplierMaster.php" id="backSupBtn" class="btn btn-secondary"
            style="width:120px;margin-left:-120px">Back</a>
    </div>
    <hr style="margin-top:5px">
</form>

<!-- Supplier Search And Export Button Start -->
<form action="" class="supplierSearchForm" style="margin-top:-20px;" method="post">
    <input type="text" class="form-control" autocomplete="off" name="supplierSearch" id="supplierSearch"
        placeholder="Search Supplier">
    <button type="submit" class="btn btn-success" name="supplierSearchBtn" id="supplierSearchBtn">Search</button>
</form>
...
<!-- Supplier Search And Export Button End -->
<!-- Supplier Table Start-->
<div style="overflow-x:auto;overflow-y:auto;max-height:220px;height:500px;" id="supplierTable">
    <table class="table text-white" style="width:3000px;height:5px;font-size:11px;">
        ...
        <tbody>
            ...
            if (isset($_POST['supplierSearchBtn'])) {

                extract($_POST);

                $_SESSION['searching_name'] = $supplierSearch;

                $querySearchSupp = "select*from suppliers where supplier_name like '%$supplierSearch%' && branch_id='$branchid' order by supplier_name";


                $resultSearchSupp = $con->query($querySearchSupp);
                $i = 1;
                while ($supplierData = $resultSearchSupp->fetch_assoc()) { ?>
            <tr>
                <td><?php echo $i++; ?></td>
                <td>
                    <a style="font-size:5px;" id="editButton"
                        href="<?php echo BASE_URL; ?>/pages/supplierMaster.php?updationid=<?php echo $supplierData['id']; ?>"
                        class="btn btn-success editBtn"><i class="fa-solid fa-pen" style="font-size:10px;"></i></a>

                    <a style="font-size:5px;" id="deleteButton"
                        href="<?php echo BASE_URL; ?>/pages/supplierMaster.php?deletionid=<?php echo $supplierData['id']; ?>"
                        onclick="return confirm('Are you sure you want to delete <?php echo $supplierData['supplier_name']; ?>?')"
                ...
                <td><?php echo $supplierData['address1']; ?></td>
                <td><?php echo $supplierData['address2']; ?></td>
                <td><?php echo $supplierData['address3']; ?></td>
                <td><?php echo $supplierData['locality']; ?></td>
                <td><?php echo $supplierData['city']; ?></td>
                <td><?php echo $supplierData['pincode']; ?></td>
                <td><?php echo $supplierData['state']; ?></td>
                <td><?php echo $supplierData['landline']; ?></td>
                <td><?php echo $supplierData['mobile']; ?></td>
                <td><?php echo $supplierData['email']; ?></td>
                <td><?php echo $supplierData['gst_no']; ?></td>
                <td><?php
                            date_default_timezone_set('Asia/kolkata');
                            echo date("d-m-Y H:i:s A", strtotime($supplierData['created_date'])); ?></td>



                <?php
                        $userIdFromSupplier = $supplierData['user_id'];
                        $branchIdFromSupplier = $supplierData['branch_id'];
                    ...
                    on
                    u.branch_id = b.id
                    where u.id = '$userIdFromSupplier' AND b.id = '$branchIdFromSupplier'
                ";
            ...
            <?php
                ...
                $querySearchSupp = "select*from suppliers where branch_id='$branchid' order by supplier_name";
            ...
            <tr>
                ...
                <td><?php echo $supplierData['email']; ?></td>
                <td><?php echo $supplierData['gst_no']; ?></td>
                <td><?php echo date("d-m-Y h:i:s A", strtotime($supplierData['created_date'])); ?></td>
                <?php
                        $userIdFromSupplier = $supplierData['user_id'];
                        $branchIdFromSupplier = $supplierData['branch_id'];
...
document.getElementById("gst").addEventListener("keypress", function(e) {
    if (e.key === "Enter") {
        e.preventDefault();
        document.getElementById("submitSupBtn").focus();
...
document.getElementById("supplierSearch").addEventListener("keypress", function(e) {
    ...
    if (e.key === "Enter") {
        e.preventDefault();
        document.getElementById("supplierSearchBtn").focus();
    }
})

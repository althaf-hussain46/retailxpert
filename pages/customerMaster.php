...
if ($_SESSION['user_role'] != "Super Admin") {
    if ($_SESSION['customer_form_create'] == 0) {
?>
<script>
document.addEventListener("DOMContentLoaded", function() {
    let submitBtn = document.getElementById('submitCusBtn');
    if (submitBtn) {
...
<?php
    ...
    if ($_SESSION['customer_form_reprint'] == 0) {
    ?>
<script>
document.addEventListener('DOMContentLoaded', function() {

    let cusSearchBtn = document.getElementById("customerSearchBtn");
    if (cusSearchBtn) {
        cusSearchBtn.setAttribute('disabled', true)
    }

    let cusSearch = document.getElementById("customerSearch");
    if (cusSearch) {
        cusSearch.setAttribute('disabled', true)
    }
...
<?php
    ...
    if ($_SESSION['customer_form_update'] == 0) {
    ?>
...
<?php
    ...
    if ($_SESSION['customer_form_delete'] == 0) {
    ?>
<script>
document.addEventListener("DOMContentLoaded", function() {
...
$_SESSION['table_name'] = "customers";
$_SESSION['field_name'] = "customer_name";
...
if (isset($_POST['createCustomer'])) {
    try {
        ...
        if (isset($name) && $name != '' && isset($state) && $state != '') {
            ...
            $querySearchCustomer = "select*from customers where customer_name = '$name'
            and branch_id = '$branchId'";
            $resultSearchCustomer = $con->query($querySearchCustomer);

            if ($resultSearchCustomer->num_rows == 0) {
                $queryCreateCustomer = "INSERT INTO customers (
                    customer_name, 
                    address1, 
                    address2, 
                    address3, 
                ...
                $stmt = $con->prepare($queryCreateCustomer);
                $stmt->bind_param(
                    "ssssssssssssii",
                    $name,
                    $address1,
                ...
                if ($stmt->execute()) {
                    ...
                    header("Location:" . BASE_URL . "/pages/customerMaster.php");
                    exit;
                } else {
                    $con->rollback();
                    $_SESSION['failure'] = "Failed to create Customer. Please try again.";
                }
            } else {
                $con->rollback();
                $_SESSION['failure'] = "Customer Already Exist! Cannot Create A Duplicate";
            }
        } else {
            $_SESSION['failure'] = "Customer Name And State Name Is Required To Create.";
        }
    } catch (mysqli_sql_exception $e) {
        // Check if it's a duplicate entry error
        $con->rollback();
        if ($e->getCode() == 1062) { // Error code for duplicate entry
            $_SESSION['failure'] = "customer Name '$name' already exists. Please enter a different customer name.";
        } else {
...
if (isset($_GET['updationId'])) {
    try {
        ...
        // Extract GET data
        extract($_GET);
        $querySearchCustomer = "select*from customers where id = '$updationId'";
        $resultSearchCustomer = $con->query($querySearchCustomer)->fetch_assoc();
        $name     = $resultSearchCustomer['customer_name'];
        $address1 = $resultSearchCustomer['address1'];
        $address2 = $resultSearchCustomer['address2'];
        $address3 = $resultSearchCustomer['address3'];
        $locality = $resultSearchCustomer['locality'];
        $city     = $resultSearchCustomer['city'];
        $pincode  = $resultSearchCustomer['pincode'];
        $state    = $resultSearchCustomer['state'];
        $landline = $resultSearchCustomer['landline'];
        $mobile   = $resultSearchCustomer['mobile'];
        $email    = $resultSearchCustomer['email'];
        $gst      = $resultSearchCustomer['gst_no'];
        ...
        if (isset($_POST['updateCustomer'])) {
            ...
            and branch_id = '$branchId' and id !='$updationId'";
                ...
                if ($resultSearchCustomer->num_rows == 0) {
                    ...
                    $qryUpdateCustomer = "UPDATE customers SET customer_name = ?, address1 = ?,
                address2 = ?, address3 = ?, locality = ?, city = ?,
                pincode = ?, state = ?, landline = ?, mobile = ?,
                email = ?, gst_no = ?, user_id = ?, branch_id = ? WHERE id = ?";
                    $stmt = $con->prepare($qryUpdateCustomer);
                    $stmt->bind_param(
                        "ssssssssssssiii",
                        $name,
                        $address1,
                    ...
                    if ($stmt->execute()) {
                        ...
                        $con->rollback();
                        $_SESSION['failure'] = "Failed to update customer. Please try again.";
                    }
                } else {
                    $con->rollback();
                    $_SESSION['failure'] = "Customer Already Exist! Cannot Create A Duplicate";
                ...
                $_SESSION['failure'] = "Customer Name or State Name cannot be empty.";
            }
        }
    } catch (mysqli_sql_exception $e) {
...
//     extract($_GET);
//     $queryDeleteCustomer = "delete from customers where id='$deletionId'";
//     $resultDeleteCustomer = $con->query($queryDeleteCustomer);
...
if (isset($_GET['deletionId'])) {
    try {
        ...
        if ($stmtDelete->execute()) {
            ...
            $_SESSION['failure'] = "Failed to delete customer. Please try again.";
        }
    } catch (mysqli_sql_exception $e) {
...
if (isset($_POST['exportButton'])) {
    extract($_POST);
    if ($fileType == "pdfFile") {
        $_SESSION['report_title'] = "Customers Report";
        $_SESSION['table_header'] = "Customer Name";
...
<form action="" class="customerForm" id="customerForm" method="post">
    <h3 style="text-align:left;font-family:Verdana, Geneva, Tahoma, sans-serif;"><span style="font-weight:bolder">Manage
        </span><span style="font-size:medium;font-weight:bold;color:gray;">Customer</span></h3>
    <hr>
    ...
    <div style="display:flex; gap:150px;margin-top:-10px;" id="beforeEditPress">
        <button type="submit" name="createCustomer" id="submitCusBtn" class="btn btn-primary"
    ...
    <div style="display:flex;gap:150px;margin-top:-38px;" id="afterEditPress">
        <button type="submit" name="updateCustomer" id="updateSupBtn" class="btn btn-primary"
            style="width:120px;">Update</button>
        <a href="<?php echo BASE_URL; ?>/pages/customerMaster.php" id="backSupBtn" class="btn btn-secondary"
            style="width:120px;margin-left:-120px">Back</a>
    </div>
    <hr style="margin-top:5px">
</form>

<!-- customer Search And Export Button Start -->
<form action="" class="customerSearchForm" style="margin-top:-20px;" method="post">
    <input type="text" class="form-control" name="customerSearch" autocomplete="off" id="customerSearch"
        placeholder="Search Customer">
    <button type="submit" class="btn btn-success" name="customerSearchBtn" id="customerSearchBtn">Search</button>
</form>
...
<!-- customer Search And Export Button End -->
<!-- customer Table Start-->
<div style="overflow-x:auto;overflow-y:auto;max-height:220px;height:500px;" id="customerTable">
    <table class="table text-white" style="width:3000px;height:5px;font-size:11px;">
        ...
        <tbody>
            ...
            if (isset($_POST['customerSearchBtn'])) {
                ...
                extract($_POST);
                $_SESSION['searching_name'] = $customerSearch;


                $querySearchCus = "select*from customers where customer_name like '%$customerSearch%' && branch_id='$branchId' order by customer_name";


                $resultSearchCus = $con->query($querySearchCus);
                $i = 1;
                while ($customerData = $resultSearchCus->fetch_assoc()) { ?>
            <tr>
                <td><?php echo $i++; ?></td>
                <td>
                    <a id="editButton"
                        href="<?php echo BASE_URL; ?>/pages/customerMaster.php?updationId=<?php echo $customerData['id']; ?>"
                        class="btn btn-success editBtn"><i class="fa-solid fa-pen" style="font-size:10px;"></i></a>

                    <a id="deleteButton"
                        href="<?php echo BASE_URL; ?>/pages/customerMaster.php?deletionId=<?php echo $customerData['id']; ?>"
                        onclick="return confirm('Are you sure you want to delete <?php echo $customerData['customer_name']; ?>?')"
                ...
                <td><?php echo $customerData['address3']; ?></td>
                <td><?php echo $customerData['locality']; ?></td>
                <td><?php echo $customerData['city']; ?></td>
                <td><?php echo $customerData['pincode']; ?></td>
                <td><?php echo $customerData['state']; ?></td>
                <td><?php echo $customerData['landline']; ?></td>
                <td><?php echo $customerData['mobile']; ?></td>
                <td><?php echo $customerData['email']; ?></td>
                <td><?php echo $customerData['gst_no']; ?></td>
                <td><?php echo date("d-m-Y h:i:s A", strtotime($customerData['created_date'])); ?></td>



                <?php
                        $userIdFromCustomer = $customerData['user_id'];
                        $branchIdFromCustomer = $customerData['branch_id'];
                    ...
                    on
                    u.branch_id = b.id
                    where u.id = '$userIdFromCustomer' AND l.id = '$branchIdFromCustomer'
                ";
            ...
            <?php
                ...
                $querySearchCus = "select*from customers where branch_id='$branchId' order by customer_name";
            ...
            <tr>
                ...
                <?php
                    ...
                    on
                    u.branch_id = b.id
                    where u.id = '$userIdFromCustomer' AND b.id = '$branchIdFromCustomer'
                ";
                        $resultUserNameBranch = $con->query($query)->fetch_assoc();
...
document.getElementById("gst").addEventListener("keypress", function(e) {
    if (e.key === "Enter") {
        e.preventDefault();
        document.getElementById("submitCusBtn").focus();
...
document.getElementById("customerSearch").addEventListener("keypress", function(e) {
    ...
    if (e.key === "Enter") {
        e.preventDefault();
        document.getElementById("customerSearchBtn").focus();
    }
})

...
}elseif(isset($_POST['sr_get_item_f4'])){
  $searchItem = $_POST['sr_get_item_f4'];
  $querySearchSalesItem  = "select i.*,sb.*
                            from items as i
                            join stock_balance as sb on i.id = sb.item_id
                            where (
...
background-image: linear-gradient(225deg, #FF3CAC 0%, #784BA0 50%, #2B86C5 100%);
  ...
  <form style="width:880px;position:absolute;top:500px;left:274px;z-index:1000;background-color: #FF3CAC;background-image: linear-gradient(225deg, #FF3CAC 0%, #784BA0 50%, #2B86C5 100%);">
  <div style="max-height: 210px; overflow-y: auto; width: 880px;">
      ...
      <thead>
        <tr style="position: sticky; top: 0; z-index: 1;font-size: 12px;background-color:#FF3CAC">
          ...
          style="background-color:#FF3CAC;"></button></th>
            ...
            <tbody>
              <?php $i =1; while($itemData = $resultSearchSalesItem->fetch_assoc()){?>
              <tr
                
                onclick="
                  
                  
                  let salesReturn = localStorage.getItem('sales_return');
                  
                  if(salesReturn == '1'){
                  let i =localStorage.getItem('sr_row_index');
                  ...
                  document.getElementById('sr_design_'+i).value ='<?php echo htmlspecialchars($itemData['design_name']); ?>';
                  document.getElementById('sr_description_'+i).value = itemDescription;
                  document.getElementById('sr_hsnCode_'+i).value ='<?php echo htmlspecialchars($itemData['hsn_code']); ?>';
                  document.getElementById('sr_tax_'+i).value ='<?php echo htmlspecialchars($itemData['tax_code']); ?>';
                  document.getElementById('sr_sellingPrice_'+i).value ='<?php echo htmlspecialchars($itemData['selling_price']); ?>';
                  document.getElementById('sr_id_'+i).value ='<?php echo htmlspecialchars($itemData['id']); ?>';
                  document.getElementById('sr_salesMan_'+i).focus();
                  document.getElementById('sr_salesMan_'+i).value = 1;
                  document.getElementById('sr_salesMan_'+i).select();
                  document.getElementById('itemInformationTable').style.display = 'none';   
              }else{

...
$querySearch = "SELECT * FROM $table_name1 WHERE $field_name1 LIKE '%$searching_name1%' && branch_id='$branchId'";
...
if ($searchResult && $searchResult->num_rows > 0) {
    ...
    // Header row
    $headers = [
        'ID', $table_header1, 'Address 1', 'Address 2', 'Address 3', 'Locality', 'City', 
        'Pincode', 'State', 'Landline', 'Mobile', 'Email', $field, 'Branch ID', 'Created Date'
    ];
    ...
    while ($data = $searchResult->fetch_assoc()) {
        ...
        $sheet->setCellValue('M' . $row, $data[$PANORGST]);
        $sheet->setCellValue('N' . $row, $data['branch_id']);
    ...
    header('Content-Disposition: attachment; filename="'.$table_name1.'_'. date('Y-m-d') . '.xlsx"');

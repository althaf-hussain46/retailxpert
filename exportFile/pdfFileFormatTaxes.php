...
$pdf->SetTextColor(0, 51, 102); // Dark blue
$pdf->Cell(190, 10, 'Taxes Report', 0, 1, 'C');
$pdf->Ln(5);
...
$pdf->Cell(20, 10, 'Tax Code', 1, 0, 'C', true);
$pdf->Cell(40, 10, 'Tax Description', 1, 0, 'C', true);
$pdf->Cell(20, 10, 'Tax (%)', 1, 0, 'C', true);
$pdf->Cell(40, 10, 'Created By', 1, 0, 'C', true);
$pdf->Cell(30, 10, 'Created Date', 1, 0, 'C', true);
$pdf->Cell(15, 10, 'ID', 1, 1, 'C', true);
...
if ($searchResult && $searchResult->num_rows > 0) {
    ...
    while ($data = $searchResult->fetch_assoc()) {
        // Add data rows
        $queryUserSearchName  = "select user_name from user_master1 where id = '$data[user_id]'";
        $resultUserSearchName = $con->query($queryUserSearchName)->fetch_assoc();
        
        $pdf->Cell(15, 10, $i++, 1, 0, 'C');
        $pdf->Cell(20, 10, $data['tax_code'], 1, 0, 'L');
        $pdf->Cell(40, 10, $data['tax_description'], 1, 0, 'L');
        $pdf->Cell(20, 10, $data['tax_percentage'], 1, 0, 'L');
        $pdf->Cell(40, 10, $resultUserSearchName['user_name'], 1, 0, 'C');
        $pdf->Cell(30, 10, date('d-m-Y',strtotime($data['created_date'])), 1, 0, 'C');
        $pdf->Cell(15, 10, $data['id'], 1, 1, 'C');
        }
    ...
    // If no data is found
    $pdf->Cell(180, 10, 'No Taxes found matching the search criteria.', 1, 1, 'C');
}

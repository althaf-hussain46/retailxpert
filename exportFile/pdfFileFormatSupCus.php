...
//$pdf->Cell(0, 10, $_SESSION['company_name']." ( ".$_SESSION['branch_name']." ) ", 0, 1, 'C'); // Company title
$pdf->Cell(0, 10, $_SESSION['branch_name'], 0, 1, 'C'); // Company title
$pdf->Ln(5);
$pdf->SetFont('Arial', '', 12);
$pdf->SetTextColor(50); // Gray text
$pdf->Cell(0, 8, $_SESSION['branch_address1']  . ', ' . $_SESSION['branch_address2'], 0, 1, 'C');
$pdf->Cell(0, 8, $_SESSION['branch_address3']  . ', ' . $_SESSION['branch_locality'], 0, 1, 'C');
$pdf->Cell(0, 8, $_SESSION['branch_city']      . ', ' . $_SESSION['branch_pinCode'], 0, 1, 'C');
$pdf->Cell(0, 8, $_SESSION['branch_landline'] . ' | ' . $_SESSION['branch_mobile'], 0, 1, 'C');
...
$pdf->SetTextColor(0, 51, 102); // Dark blue
$pdf->Cell(0, 10, $report_title1, 0, 1, 'C');
$pdf->Ln(5);

// Subtitle with Date
$pdf->SetFont('Arial', '', 12);
$pdf->SetTextColor(50); // Gray text
$pdf->Cell(0, 10, ' Generated on: ' . date('d-m-Y').'  And  Generated by ' . $_SESSION['user_name'] . ' - ' . $_SESSION['branch_name'], 0, 1, 'C');
...
$columns = [10, 50, 100, 100, 30, 28]; // Adjusted column widths
$headers = ['S.No', $_SESSION['table_header'], 'Address', 'Contacts', $field, 'Branch Name'];
...
if ($searchResult && $searchResult->num_rows > 0) {
    ...
    while ($data = $searchResult->fetch_assoc()) {
        $queryBranchName = "SELECT branch_name FROM branches WHERE id = '$data[branch_id]'";
        $resultBranchName = $con->query($queryBranchName)->fetch_assoc();
        ...
        $pdf->Cell($columns[1], $rowHeight, $data[$field_name1], 1, 0, 'L');
        ...
        // GST No.
        $pdf->Cell($columns[4], $rowHeight, $data[$PANORGST], 1, 0, 'L');

        // Location ID
        $pdf->Cell($columns[5], $rowHeight, $resultBranchName['branch_name'], 1, 1, 'C');
    }



} else {
    $pdf->Cell(array_sum($columns), 10, 'No  found matching the search criteria.', 1, 1, 'C');
}

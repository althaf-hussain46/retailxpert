...
$querySearchPurchaseReturnItem  = "SELECT 
                                ...
                                INNER JOIN items as i ON pri.pr_item_id = i.id
                                WHERE pri.branch_id = '$userBranchId' and date(pri.pr_grn_date) BETWEEN '$fromDate' AND '$toDate'";
    $resultSearchPurchaseReturnItem = $con->query($querySearchPurchaseReturnItem);
    // $querySearchPurchaseReturnSum = "
    // SELECT ps.*, s.* 
    ...
    WHERE prs.pr_grn_number = '20242000030'
    AND prs.branch_id = '$userBranchId' and date(prs.pr_grn_date) BETWEEN '$fromDate' and '$toDate'";
$resultSearchPurchaseReturnSum = $con->query($querySearchPurchaseReturnSum);
...
$totaldeduction = 0;
$totalnetamount = 0;
while($resultPurchaseReturnSummary = $resultSearchPurchaseReturnSum->fetch_assoc()){
    $totalcgst         = $totalcgst+$resultPurchaseReturnSummary['pr_cgst_amount'];
    $totalsgst         = $totalsgst+$resultPurchaseReturnSummary['pr_sgst_amount'];
    $totaligst         = $totaligst+$resultPurchaseReturnSummary['pr_igst_amount']||0;
    $totaladdon        = $totaladdon+$resultPurchaseReturnSummary['pr_addon'];
    $totaldeduction    = $totaldeduction+$resultPurchaseReturnSummary['pr_deduction'];
    $totalnetamount    = $totalnetamount+$resultPurchaseReturnSummary['pr_net_amount'];    
...
// $purchasesResult = $con->query($queryPurchases);
    ...
    $sheet->setCellValue('A6', 'P U R C H A S E   R E T U R N   I T E M');
    ...
    // $sheet->setCellValue('A8', $resultSearchPurchaseReturnSum['supplier_name']);
    ...
    // $sheet->setCellValue('J9', $resultSearchPurchaseReturnSum['grn_number']);
    ...
    // $sheet->setCellValue('L9', $resultSearchPurchaseReturnSum['invoice_number']);
    ...
    // $sheet->setCellValue('O9', $resultSearchPurchaseReturnSum['dc_number']);
    ...
    // $sheet->setCellValue('J10', date('d-m-Y',strtotime($resultSearchPurchaseReturnSum['grn_date'])));
    ...
    // $sheet->setCellValue('L10', date('d-m-Y',strtotime($resultSearchPurchaseReturnSum['invoice_date'])));
    ...
    // $sheet->setCellValue('O10', date('d-m-Y',strtotime($resultSearchPurchaseReturnSum['dc_date'])));
    ...
    // Headers Row
    $headers = ['Sno', 'PR Number', 'PRN Date', 'Product', 'Brand', 'Design', 'Color', 'Batch',
    ...
    $totalqty = 0;
    $totalamount = 0;
    while ($data = $resultSearchPurchaseReturnItem->fetch_assoc()) {
        ...
        $sheet->setCellValue('P' . $row, $data['pr_land_cost']);
        $sheet->setCellValue('Q' . $row, $data['pr_margin']);
        $sheet->setCellValue('R' . $row, $data['pr_item_qty']);
        $sheet->setCellValue('S' . $row, $data['pr_item_amount']);
        
        $totalqty = $totalqty+$data['pr_item_qty'];
        $totalamount = $totalamount+$data['pr_item_amount'];
...
$sheet->getStyle('R' . $row . ':S' . $row)->applyFromArray([
    ...
    header('Content-Disposition: attachment; filename="PurchaseReturnItem_' . date('Y-m-d') . '.xlsx"');

...
$pdf->Cell(190, 8,  $_SESSION['branch_landline'].' | '.$_SESSION['branch_mobile'], 0, 1, 'C');
...
if($_SESSION['user_role'] == "Super Admin"){
    ...
    $pdf->Cell(30, 10, 'User Name', 1, 0, 'C', true);
    $pdf->Cell(40, 10, 'User Password', 1, 0, 'C', true);
    $pdf->Cell(30, 10, 'User Role', 1, 0, 'C', true);
    $pdf->Cell(30, 10, 'Location Id', 1, 0, 'C', true);
    $pdf->Cell(40, 10, 'Created Date', 1, 1, 'C', true);
}else{
    $pdf->Cell(20, 10, 'ID', 1, 0, 'C', true);
    $pdf->Cell(40, 10, 'User Name', 1, 0, 'C', true);
    $pdf->Cell(40, 10, 'User Role', 1, 0, 'C', true);
    $pdf->Cell(40, 10, 'Location Id', 1, 0, 'C', true);
    $pdf->Cell(50, 10, 'Created Date', 1, 1, 'C', true);
...
if ($searchResult && $searchResult->num_rows > 0) {
    while ($data = $searchResult->fetch_assoc()) {
        ...
        if($_SESSION['user_role'] == "Super Admin"){
            ...
            $pdf->Cell(30, 10, $data['user_name'], 1, 0, 'L');
            $pdf->Cell(40, 10, $data['user_pass'], 1, 0, 'L');
            $pdf->Cell(30, 10, $data['user_role'], 1, 0, 'L');
            $pdf->Cell(30, 10, $data['branch_id'], 1, 0, 'L');
            $pdf->Cell(40, 10, $data['created_date'], 1, 1, 'C');
        }else{
            $pdf->Cell(20, 10, $data['id'], 1, 0, 'C');
            $pdf->Cell(40, 10, $data['user_name'], 1, 0, 'L');
            $pdf->Cell(40, 10, $data['user_role'], 1, 0, 'L');
            $pdf->Cell(40, 10, $data['branch_id'], 1, 0, 'L');
            $pdf->Cell(50, 10, $data['created_date'], 1, 1, 'C');
        }
    ...
    // If no data is found
    $pdf->Cell(190, 10, 'No Users found matching the search criteria.', 1, 1, 'C');
}
